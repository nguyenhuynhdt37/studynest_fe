...
const CreateTopic = () => {
  ...
  // Fetch all categories for dropdown (paginated)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("📤 Gửi request tạo topic:", payload);

      const response = await api.post("/admin/topics", payload);

      console.log("✅ Response từ server:", response.data);

      showToast.success("Đã tạo topic thành công!");
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      if (typeof generatedDescription === "string") {
        ...
        if (generatedDescription.trim().startsWith("{") || generatedDescription.trim().startsWith('"')) {
          try {
      ...
      } else if (generatedDescription && typeof generatedDescription === "object") {
