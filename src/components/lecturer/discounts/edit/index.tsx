...
// Helper: Convert ISO datetime to datetime-local format (theo timezone local)
const isoToDatetimeLocal = (isoString: string): string => {
  if (!isoString) return "";
  try {
    // Parse ISO string (có thể là UTC hoặc có timezone)
    const date = new Date(isoString);

    // Lấy local time (theo timezone của browser, thường là UTC+7 cho VN)
...
const datetimeLocalToISO = (datetimeLocal: string): string => {
  ...
  try {
    // datetime-local không có timezone, nó là local time (theo timezone của browser)
    // Tạo Date object từ local time string
    // JavaScript sẽ tự động parse nó như local time
    ...
    // Convert sang ISO string (sẽ tự động convert từ local time sang UTC)
    // Ví dụ: "2025-11-15T16:46" (UTC+7) -> "2025-11-15T09:46:00.000Z" (UTC)
...
export default function EditDiscount() {
  ...
  const { data: discountData, error: discountError } =
    useSWR<DiscountEditResponse>(
      discountId ? `/lecturer/discounts/${discountId}/edit` : null,
      async (url) => {
  ...
  // Fetch all courses to match with targets
  const { data: allCoursesData } = useSWR<DiscountCoursesResponse>(
    discountData && discountData.applies_to === "course"
  ...
  // Initialize form data when discount data is loaded
  useEffect(() => {
    if (discountData) {
      // Find selected courses from targets
      if (discountData.applies_to === "course" && allCoursesData?.courses) {
        const selectedCourseIds = discountData.targets.map((t) => t.course_id);
        const courses = allCoursesData.courses.filter((c) =>
      ...
      setFormData({
        ...
        is_hidden: discountData.is_hidden || false,
        applies_to: discountData.applies_to || "course",
        discount_type: discountData.discount_type || "fixed",
        percent_value: discountData.percent_value,
        fixed_value: discountData.fixed_value,
        usage_limit: discountData.usage_limit,
        per_user_limit: discountData.per_user_limit || 1,
        start_at: isoToDatetimeLocal(discountData.start_at),
        end_at: isoToDatetimeLocal(discountData.end_at),
        targets: discountData.targets.map((t) => ({
          course_id: t.course_id,
        })),
      });
    }
  }, [discountData, allCoursesData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/lecturer/discounts/${discountId}`, payload);
  ...
  if (discountError || !discountData) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-4">Không thể tải dữ liệu mã giảm giá</p>
          <button
            onClick={() => router.back()}
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">Cập nhật thông tin mã giảm giá</p>
        </div>
